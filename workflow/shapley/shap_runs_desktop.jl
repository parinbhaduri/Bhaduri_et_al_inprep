...
flood_shock = 1982 #We're collecting data up till the flood event occurs, so the flood shock year doesnt matter
...
# Set up pop shares data file 
pop_share_file = joinpath(data_dir,"study_pop_share_data.h5")
...
## First start by collecting the same data during the initialization
init_pop_share_file = joinpath(data_dir,"study_init_pop_share_data.h5")

h5open(init_pop_share_file, "w") do file
    # Create datasets with chunking for efficient I/O
    chunk_size = (1,9,7) 
        
    # Main data array: (runs, agents, years, variables)
    create_dataset(file, "pop_share_data", Float64, (1,n_agents*9, 7), 
                    chunk=chunk_size, deflate=9, shuffle=true
    )
...
output_params_dict = Dict(output_params .=> combs[0]) #Only need one parameter value with pop_no = 0, since agents all initialized the same
init_model = PhilPopABM(;output_params_dict...)
#Run
init_pop_df = DataFrame() 
init_pop_df = shock_pop_shares(init_model)
save_pop_share_data!(init_pop_share_file, 1, init_pop_df)
