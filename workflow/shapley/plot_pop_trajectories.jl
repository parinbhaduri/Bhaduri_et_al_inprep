...
fld_year = 2011
flpn_norm_low = DataFrame(CSV.File(joinpath(out_dir, "post_process","population","$(fld_year)_model_outcome_flpn_pop_norm_low.csv")))[:,1:21]
flpn_norm_high = DataFrame(CSV.File(joinpath(out_dir, "post_process","population","$(fld_year)_model_outcome_flpn_pop_norm_high.csv")))[:,1:21]
...
text!(ax1, 4.5, 23, text=rich("Flood Shock Occurrence", font = :italic), align = (:left, :center), fontsize = 22)
...
axislegend(ax1, [elem_1, elem_2, elem_3, elem_4] , ["Low Income Population", "Low Income Trajectories", 
    "High Income Population", "High Income Trajectories"], nbanks = 2,
