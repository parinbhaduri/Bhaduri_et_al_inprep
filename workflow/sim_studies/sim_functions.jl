...
function simul_market(adf, mdf, group_col; leg = :outertopright, color = palette(:BrBg), lim =(0,600), price_lim =(1e5,1.5e6))
    ...
    move_ind = mdf.time .!= 0 
    push!(plots, Plots.plot(mdf.time[move_ind], mdf.moved_low[move_ind], group = mdf[:, group_col][move_ind], legend=leg, palette = color, linewidth=2, ylimits=lim, title="Low Income Movers"))
    push!(plots, Plots.plot(adf.time, adf.mean_price_low_BG, group = adf[:, group_col], legend=leg, palette = color, linewidth=2, ylimits=price_lim, title="Low Income House Price"))
    
    push!(plots, Plots.plot(mdf.time[move_ind], mdf.moved_med[move_ind], group = mdf[:, group_col][move_ind], legend=leg, palette = color, linewidth=2, ylimits=lim, title="Medium Income Movers"))
    push!(plots, Plots.plot(adf.time, adf.mean_price_med_BG, group = adf[:, group_col], legend=leg, palette = color, linewidth=2, ylimits=price_lim, title="Medium Income House Price"))
    
    push!(plots, Plots.plot(mdf.time[move_ind], mdf.moved_high[move_ind], group = mdf[:, group_col][move_ind], legend=leg, palette = color, linewidth=2, ylimits=lim, title="High Income Movers"))
    push!(plots, Plots.plot(adf.time, adf.mean_price_high_BG, group = adf[:, group_col], legend=leg, palette = color, linewidth=2, ylimits=price_lim, title="High Income House Price"))
