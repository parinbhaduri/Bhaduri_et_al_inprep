...
function PhilSim(bg_df, pop_df, flood_df;no_of_years::Int64, start_year::Int64, perc_growth::Float64, flood_coefficient::Float64, 
    ...
    penalty::Float64, util_coef::Dict, house_budget_mode::String, house_budget_perc::Float64, dist_param::Vector, seed::Int64
) 
...
function phil_model(; flood_rec = phil_flood_record, perc_growth=0.01, flood_coefficient=0.5, 
    ...
    penalty=0.5, house_budget_mode="rhea", house_budget_perc=0.33, dist_param = [0.3, 0.4, 0.3],
    ...
    model = PhilSim(phil_bg, phil_cbsa_base_pop, flood_rec;no_of_years=Int(no_of_years), start_year=Int(start_year), perc_growth=perc_growth,
                 ...
                 house_budget_mode=house_budget_mode, house_budget_perc=house_budget_perc, dist_param = dist_param
    )
    return model
end
...
function phil_util(;prop_l=prop_l, env_amen_l=env_amen_l, 
    ...
    model = PhilSim(phil_bg, phil_cbsa_base_pop, synth_flood_record;no_of_years=Int(no_of_years), start_year=Int(start_year), perc_growth=0.01, flood_coefficient=0.5, 
         risk_averse=0.5, flood_mem=10, base_move=0.01, build_inc_perc=0.10, price_inc_perc=0.10, dist_param = [0.3, 0.4, 0.3], 
         penalty=0.5, util_coef=util_coef, seed=Int(seed), house_budget_mode="rhea", house_budget_perc=0.33
    )
    return model
end
